<<<<<<< HEAD
"""
Wind Power Forecasting Model for PVGRIDBALANCER
Implements Temporal Convolutional Network (TCN) for wind generation prediction
"""
...
class WindForecaster:
    """Wind power generation forecasting using TCN"""
    ...
    def __init__(self, preprocessed_path='data/preprocessed', models_path='models'):
        ...
        self.feature_cols = []
        self.target_col = 'Power Generation '
    ...
    def prepare_features(self, df):
        """Select relevant features for wind forecasting"""
        print("\n=== Preparing Wind Features ===")
        
        # Wind-specific features
        wind_features = [
            'Air Density', 'Wind Speed ',
            'wind_power_coefficient_cp', 'wind_power_per_speed',
            'hour_sin', 'hour_cos', 'day_sin', 'day_cos',
            'day_of_week', 'month'
        ]
        
        # Add lag features
        lag_features = [col for col in df.columns if 'wind_lag' in col]
        rolling_features = [col for col in df.columns if 'wind_rolling' in col]
        
        # Combine all features
        self.feature_cols = [col for col in wind_features + lag_features + rolling_features 
    ...
    def train(self, X, y, epochs=50, batch_size=32, validation_split=0.2):
        """Train the TCN model"""
        print("\n=== Training Wind Forecasting Model ===")
    ...
    def save_model(self):
        ...
        if self.model and DEPS_AVAILABLE:
            model_file = self.models_path / 'wind_tcn_model.keras'
            self.model.save(model_file)
            print(f"\n✓ Saved model to {model_file}")
            
            # Save scalers
            scaler_file = self.models_path / 'wind_scalers.pkl'
    ...
    def run(self, epochs=50):
        """Execute full wind forecasting pipeline"""
        print("="*60)
        print("Wind Forecasting Model Training")
        print("="*60)
        ...
        print("\n" + "="*60)
        print("✓ Wind Forecasting Model Training Complete!")
        print("="*60)
